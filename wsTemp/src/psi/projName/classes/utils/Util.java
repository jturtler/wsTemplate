...
public class Util {
	...
	private static void sendRequestHTTP( DataStore dataStore )  throws Exception
	{
		...
		{		
			...
			else if ( dataStore.request_ContentType.equals( "JSON" ) )
	        {	
			...
			if ( !dataStore.request_Auth.isEmpty() )
	        {		
				if ( dataStore.request_Auth.equals( "BASIC_AUTH" ) )
				{
	...
	public static JSONObject getJsonFromInputStream( InputStream is ) throws Exception
	{
		...
		{
			Util.outputErr( "Failed on getJsonFromInputStream: " + ex.getMessage() );
			throw ex;
		} 
